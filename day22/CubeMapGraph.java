...
// Note that this only works for my specific data
...
class CubeMapGraph {
  ...
  private ArrayList<CubeMapCharacter> mapCharacters;
  ...
  // Constructor
  CubeMapGraph(String mapData, char wallChar, char openChar, char emptyChar) {
    ...
    for (String dataLine : splitData) {
      ...
      for (int i = 0; i < sizeX; i++) {
        // Determine which cube zone it is in:
        int zone = 0;
        if (row < 50) {
          if (i < 100) {
            zone = 1;
        ...
        try {
          ...
          if (currChar == wallChar) {
            lineTiles.add(new WallTile(currChar, i, row, zone));
          } else if (currChar == openChar) {
            lineTiles.add(new OpenTile(currChar, i, row, zone));
          } else {
            lineTiles.add(null);
          }
        } catch (Exception e) {
  ...
  public void addCharacter(CubeMapCharacter newCharacter) {
  ...
  public boolean moveCharacter(CubeMapCharacter currCharacter, char direction) {
  ...
  public int[] characterLocation(CubeMapCharacter currCharacter) {
    int charIndex = this.mapCharacters.indexOf(currCharacter);
